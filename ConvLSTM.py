...
class CLSTM_cell(nn.Module):
    ...
    def __init__(self, shape, input_chans, filter_size, num_features):
        ...
        self.padding = int(filter_size // 2) # same padding
    ...
    def init_hidden(self,x):
        return (Variable(torch.zeros(x.shape[0], self.num_features, self.shape[0], self.shape[1])).cuda(),
                Variable(torch.zeros(x.shape[0], self.num_features, self.shape[0], self.shape[1])).cuda())
...
class CLSTM(nn.Module):
    ...
    def __init__(self, shape, input_chans, filter_size, num_features, num_layers):
        ...
        cell_list=[]
        cell_list.append(CLSTM_cell(self.shape, self.input_chans, self.filter_size,
                                    self.num_features).cuda()) # the first one has a different number of input channels
    ...
    def forward(self, input, hidden_state):
        ...
        args:
          ...
          input is the tensor of shape [seq_len, Batch, Channels, H, W]

        """

        current_input = input.transpose(0, 1).contiguous() # now is seq_len,B,C,H,W
        next_hidden = [] # hidden states(h&c)
